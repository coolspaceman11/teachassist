...
const clearServerDependentCache = async () => {
  ...
  if (storedCoursesJson) {
    try {
      const parsedCourses = JSON.parse(storedCoursesJson) as { subjectId?: string | number }[];
      await Promise.all(
        parsedCourses
...
export default function PersonalizationAdvancedScreen() {
  const router = useRouter();
  const { activeTone, isDark } = useTheme();
  const { afoolOverrideEnabled, setAFoolOverrideEnabled } = useAFoolVisualGrades();
  const [serverValue, setServerValue] = useState("");
  const [savedServer, setSavedServer] = useState(DEFAULT_TEACHASSIST_SERVER_ORIGIN);
  const [markNotificationsEnabled, setMarkNotificationsEnabled] = useState(false);
  ...
  const showLiquidGlassAppearanceWarning = systemColorScheme === "light" && liquidGlassEnabled && isDark;

  useEffect(() => {
    const loadSettings = async () => {
      const [currentServer, notificationSettings, storedLiquidGlassEnabled] = await Promise.all([
        getTeachAssistServerOrigin(),
  ...
  const submitExperimentCode = async (
    event?: NativeSyntheticEvent<TextInputSubmitEditingEventData>,
  ) => {
    const submittedCode = event?.nativeEvent.text?.trim() ?? experimentCode.trim();

    if (submittedCode.toUpperCase() === "GPT") {
      const enabled = await toggleGPTAccess();
      setExperimentCode("");
    ...
    if (submittedCode === "a secret code" || submittedCode === "A secret code") {
  ...
  return (
    <View className={`flex-1 ${isDark ? "bg-dark1" : "bg-light1"}`}>
      ...
      >
        <Text className={`text-4xl font-semibold ${isDark ? "text-appwhite" : "text-appblack"}`}>
          Advanced
        </Text>
        <Text className={`mt-1 text-base leading-6 ${isDark ? "text-appgraylight" : "text-appgraydark"}`}>
          Settings for nerds
        </Text>

        <View className="mt-6">
          <Text className="text-2xl font-bold text-baccent mb-4">Notifications</Text>
          <LiquidGlassView className="rounded-2xl overflow-hidden" fallbackBackgroundColor={activeTone.bg3} glassTintColor={activeTone.bg2} glassEffectStyle="clear">
            <View className={`px-4 py-4 flex-row justify-between items-center ${markNotificationsEnabled ? "" : "opacity-50"}`}>
              <View className="flex-1 pr-3">
                <Text className={`${isDark ? "text-appwhite" : "text-appblack"} text-base font-semibold`}>Notify When No Change In Mark</Text>
                <Text className={`${isDark ? "text-appgraylight" : "text-appgraydark"} text-sm mt-1`}>Alert me after a background check finds no updates. Requires mark change notifications ON.</Text>
              </View>
              <AppToggle value={notifyWhenNoChanges} disabled={!markNotificationsEnabled} onValueChange={toggleNoChangeAlerts} />
            </View>
          </LiquidGlassView>
        </View>

        <View className="mt-6">
          <Text className="text-2xl font-bold text-baccent mb-4">Interface</Text>
          <LiquidGlassView className="rounded-2xl overflow-hidden" fallbackBackgroundColor={activeTone.bg3} glassTintColor={activeTone.bg2} glassEffectStyle="clear">
            <View className={`px-4 py-4 flex-row justify-between items-center ${isAndroid ? "opacity-50" : ""}`}>
              <View className="flex-1 pr-3">
                <View className="flex-row items-center">
                  <Text className={`${isDark ? "text-appwhite" : "text-appblack"} text-base font-semibold`}>Use Liquid Glass</Text>
                  <Text className="bg-info rounded-full px-3 ml-2 font-semibold text-appwhite">Beta</Text>
                </View>
                <Text className={`${isDark ? "text-appgraylight" : "text-appgraydark"} text-sm mt-1`}>
                  {isAndroid
                    ? "Liquid glass is not available on Android."
                    : showLiquidGlassAppearanceWarning
                      ? "Enable some liquid glass features in iOS and iPadOS. If your device stays in Light Mode while the app is in Dark Mode, things might look weird."
                      : "Enable some liquid glass features in iOS and iPadOS."}
                </Text>
              </View>
              <AppToggle value={liquidGlassEnabled} disabled={isAndroid} onValueChange={toggleLiquidGlass} />
            </View>
          </LiquidGlassView>
        </View>

        <View className="mt-6">
          <Text className="text-2xl font-bold text-baccent mb-4">Custom Server</Text>
          <LiquidGlassView className="rounded-2xl overflow-hidden" fallbackBackgroundColor={activeTone.bg3} glassTintColor={activeTone.bg2} glassEffectStyle="clear">
            <View className="px-4 pt-5 pb-3">
              <Text className={`${isDark ? "text-appwhite" : "text-appblack"} font-semibold`}>Modify TA RPC client</Text>
              <AppTextInput
                value={serverValue}
              ...
              />
              <Text className={`${isDark ? "text-appgraylight" : "text-appgraydark"} text-sm mt-3`}>
                Default is {DEFAULT_TEACHASSIST_SERVER_ORIGIN}.{"\n"}Currently using {savedServer}.{"\n"}Format as URL.
              </Text>
            </View>
            <View className={`${isDark ? "bg-dark4" : "bg-light4"} h-px mx-4 mb-1`} />
            <View className="px-4 py-4 flex-row gap-3">
              <LiquidGlassButton
                containerStyle={{ flex: 1 }}
                contentStyle={{ borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, alignItems: "center", justifyContent: "center" }}
                ...
                onPress={async () => {
                  try {
                    ...
                    AppAlert.alert("Invalid Server", error instanceof Error ? error.message : "Enter a valid server host.", { icon: AlertIcon.error });
                  }
                }}
              >
                <Text className={`font-semibold ${isDark ? "text-appblack" : "text-appwhite"}`}>Save</Text>
              </LiquidGlassButton>
              <LiquidGlassButton
                containerStyle={{ flex: 1 }}
                contentStyle={{ borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, alignItems: "center", justifyContent: "center" }}
                ...
                onPress={() => saveServer(null)}
              >
                <Text className={`font-semibold ${isDark ? "text-appwhite" : "text-appblack"}`}>Reset Default</Text>
              </LiquidGlassButton>
            </View>
          </LiquidGlassView>
        </View>
        ...
        <View className="mt-6">
          ...
          <LiquidGlassView className="rounded-2xl overflow-hidden" fallbackBackgroundColor={activeTone.bg3} glassTintColor={activeTone.bg2} glassEffectStyle="clear">
            <View className="px-4 py-4">
              <Text className={`${isDark ? "text-appwhite" : "text-appblack"} text-base font-semibold`}>Share Device & App Info</Text>
              <Text className={`${isDark ? "text-appgraylight" : "text-appgraydark"} text-sm mt-1 mb-3`}>Share a snapshot of app state and device details to help diagnose issues.</Text>
              <LiquidGlassButton
                contentStyle={{ borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, alignItems: "center", justifyContent: "center" }}
                glassTintColor={activeTone.accent}
                fallbackBackgroundColor={activeTone.accent}
                onPress={async () => {
                  const [marksLastRetrieved, lastAuthTime, schoolName, customServer, notifsEnabled, hideUnavailable, tapToReveal, messagesMode, hapticsEnabled] = await Promise.all([
                  ...
                  const fmtDate = (raw: string | null) => {
                    if (!raw) return "never";
                    const ms = /^\d+$/.test(raw) ? parseInt(raw, 10) : new Date(raw).getTime();
                  ...
                  try {
                    ...
                    AppAlert.alert("Failed to share debug info.", undefined, { icon: AlertIcon.error });
                  }
                }}
              >
                <Text className={`font-semibold ${isDark ? "text-appblack" : "text-appwhite"}`}>Share Debug Info</Text>
              </LiquidGlassButton>
            </View>
          </LiquidGlassView>
        </View>
        ...
        <View className="mt-6">
          <LiquidGlassView className="rounded-2xl overflow-hidden" fallbackBackgroundColor={activeTone.bg3} glassTintColor={activeTone.bg2} glassEffectStyle="clear">
            <View className="px-4 pt-2 pb-4">
              <Text className={`${isDark ? "text-appwhite" : "text-appblack"} font-semibold mt-2`}>Enable experiments</Text>
              <AppTextInput
                value={experimentCode}
                onChangeText={setExperimentCode}
                onSubmitEditing={submitExperimentCode}
                autoCapitalize="none"
                autoCorrect={false}
                returnKeyType="done"
                ...
                className={`mt-3 rounded-xl px-4 py-4 ${isDark ? "bg-dark4 text-appwhite" : "bg-light4 text-appblack"}`}
              />
              <Text className={`${isDark ? "text-appgraylight" : "text-appgraydark"} text-sm ${afoolOverrideEnabled ? "pt-2" : ""}`}>
